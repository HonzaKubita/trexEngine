function solveWallCollisions(particles, enviroment, velocityLoose) {
  ...
  particles.forEach(particle => { // Repeat for each particle
    if(particle.position.x - particle.texture.width / 2 < 0) { // If particle is below the ground
    ...
    else if (particle.position.x + particle.texture.width / 2 > enviroment.width) { // If particle is above the ground
    ...
    if(particle.position.y - particle.texture.height / 2 < 0) { // If particle is behind left wall
    ...
    else if(particle.position.y + particle.texture.height / 2 > enviroment.height) { // If particle is behind right wall
